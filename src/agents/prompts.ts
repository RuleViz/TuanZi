...
export function searcherSystemPrompt(input: {
  ...
  return [
    ...
    "    <rule>Prefer structured exploration order: ls -> glob -> grep -> read.</rule>",
...
export function coderSystemPrompt(input: {
  ...
  return [
    ...
    "    <rule>Prefer tool sequence: ls -> glob -> grep -> read before edit/write; use bash mainly for verification.</rule>",
