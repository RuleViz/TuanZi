...
export class TurnCheckpointManager {
  ...
  async createCheckpoint(turnId: string, turnIndex: number, userMessage: string): Promise<TurnCheckpoint | null> {
    try {
      ...
      if (statusResult.ok && !statusResult.stdout.trim()) {
        // No changes since last checkpoint; reuse previous
      ...
      this.logger.warn(`Failed to create checkpoint: ${msg}`);
      return null;
    }
  }
  ...
  async restore(turnId: string): Promise<{ restoredFiles: number; removedFiles: number } | null> {
    try {
      ...
      const checkoutResult = await this.runGit(
        ["checkout", checkpoint.commitHash, "--", "."],
        this.shadowRoot
      );
      ...
      // Reset shadow HEAD to the restored commit
      await this.runGit(["reset", "--hard", checkpoint.commitHash], this.shadowRoot);

      return stats;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to restore checkpoint: ${msg}`);
      return null;
    }
  }
  ...
  async diff(turnId: string): Promise<string | null> {
    try {
      ...
      // First sync current workspace into shadow
      await this.syncWorkspaceToShadow();
      await this.runGit(["add", "-A"], this.shadowRoot);

      const diffResult = await this.runGit(
        ["diff", "--cached", checkpoint.commitHash],
        this.shadowRoot
      );
      if (!diffResult.ok) {
        return null;
      ...
      // Reset the staging area after diff
      await this.runGit(["reset", "HEAD"], this.shadowRoot);

      return diffResult.stdout || "(no changes)";
    } catch {
      return null;
    }
  }
  ...
  private async syncShadowToWorkspace(): Promise<{ restoredFiles: number; removedFiles: number }> {
    ...
    // 1) Copy all files from shadow to workspace
    ...
    // 2) Remove files in workspace that don't exist in shadow
    const shadowSet = new Set(shadowFiles.map((f) => normalizePath(f)));
    ...
    // 3) Remove empty directories in workspace that resulted from file removal
...
async function syncDirectory(src: string, dest: string, excludes: string[]): Promise<void> {
  ...
  const srcSet = new Set(files.map((f) => normalizePath(f)));
