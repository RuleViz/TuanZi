...
export interface ProviderSettingsController {
  ...
  normalizeProviderModelList: (input: unknown) => ProviderModelItem[];
...
export function createProviderSettingsController(input: ProviderSettingsDeps): ProviderSettingsController {
  ...
  const normalizeProviderModelList = (value: unknown): ProviderModelItem[] => {
  ...
  const normalizeProviderDraft = (value: Partial<ProviderConfig>): ProviderConfig => {
    return {
      ...
      type: input.normalizeOptionalString(value.type) ?? input.defaultProviderType,
      ...
      models: normalizeProviderModelList(value.models),
